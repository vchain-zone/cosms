...
describe('Cosm test', async () => {
  ...
  before('Connect', async () => {
    ...
    client = await SigningStargateClient.connectWithSigner(rpcUrl, wallet, options);
  ...
  before('Setup delegation info', async () => {
    ...
    const queryValidatorsResponse = await cosm.cosmos.staking.block(currentBlock - 100).query.Validators(
    ...
    const validatorDelegationsResponse = await cosm.cosmos.staking.block(currentBlock - 100).query.ValidatorDelegations(
  ...
  describe('test client', async () => {
    it('should lookup MsgCustom success', function() {
      ...
      expect(openedClient.registry.lookupType('/custom.MsgCustom')).is.equal(MsgSend);
    });
  });
  describe('Get blockchain information', async () => {
    it('should get blockchain info', async function() {
      ...
      expect(balance, `balance of address ${address}: ${balance}`).is.not.eq(undefined);
    });
  });
  ...
  describe('Test auth query', async () => {
    ...
    it('should get bank accounts', async () => {
      ...
      const _account = await cosm.cosmos.auth.query.Account({ address: delegation.delegatorAddress });
  ...
  describe('Test bank query', async () => {
    ...
    it('should get bank info', async () => {
      ...
      const totalSupplyResponse = await cosm.cosmos.bank.block(currentBlock - 10).query.TotalSupply({});
      // console.log(totalSupplyResponse);
      expect(totalSupplyResponse).is.not.empty;

      const paramsResponse = await cosm.cosmos.bank.block(currentBlock - 11).query.Params({});
      // console.log(paramsResponse);
      expect(paramsResponse).is.not.empty;

      const denomsMetadataResponse = await cosm.cosmos.bank.block(currentBlock - 12).query.DenomsMetadata({});
    ...
    it('should get bank account info', async () => {
      ...
      const allBalancesResponse = await cosm.cosmos.bank.block(currentBlock - 12).query.AllBalances({ address: delegation.delegatorAddress });
  ...
  describe('Test staking query', async () => {
    ...
    it('should get staking info', async function() {
      ...
      const queryValidatorsResponse = await staking.block(currentBlock - 100).query.Validators(
        { status: BondStatus[BondStatus.BOND_STATUS_BONDED] }
      );
      // console.log(queryValidatorsResponse);
      ...
      const validatorResponse = await staking.block(currentBlock - 100).query.Validator(
        { validatorAddr: validatorAddr }
      );
      // console.log(validatorResponse);
      expect(validatorResponse).is.not.eq(undefined);

      const validatorDelegationsResponse = await staking.block(currentBlock - 100).query.ValidatorDelegations(
        { validatorAddr: validatorAddr }
      );
      ...
      let queryHistoricalInfoResponse = await staking.query.HistoricalInfo({ height: Long.fromNumber(currentBlock) });
    ...
    it('should get delegator info', async () => {
      ...
      let queryDelegatorDelegationsResponse = await staking.query.DelegatorDelegations({ delegatorAddr: account.address });
      // console.log(queryDelegatorDelegationsResponse);
      expect(queryDelegatorDelegationsResponse).is.not.empty;

      let queryDelegatorUnbondingDelegationsResponse = await staking.query.DelegatorUnbondingDelegations({ delegatorAddr: account.address });
      // console.log(queryDelegatorUnbondingDelegationsResponse);
      expect(queryDelegatorUnbondingDelegationsResponse).is.not.empty;

      let queryDelegatorValidatorsResponse = await staking.query.DelegatorValidators({ delegatorAddr: account.address });
      ...
      const delegation = validatorDelegationsResponse.delegationResponses[0].delegation;
      ...
      let queryDelegatorValidatorResponse = await staking.query.DelegatorValidator({
        delegatorAddr: delegation.delegatorAddress,
        validatorAddr: delegation.validatorAddress
      });
  ...
  describe('Test distribution query', async () => {
    ...
    it('should get distribution info', async function() {
      ...
      let queryValidatorOutstandingRewardsResponse = await distribution.query.ValidatorOutstandingRewards({ validatorAddress: validatorAddr });
      // console.log("queryValidatorOutstandingRewardsResponse");
      // console.log(queryValidatorOutstandingRewardsResponse);

      let queryValidatorCommissionResponse = await distribution.query.ValidatorCommission({ validatorAddress: validatorAddr });
      // console.log("queryValidatorCommissionResponse");
      // console.log(queryValidatorCommissionResponse);

      let queryDelegationRewardsResponse = await distribution.query.DelegationRewards(delegation);
      // console.log("queryDelegationRewardsResponse");
      // console.log(queryDelegationRewardsResponse);

      let queryDelegationTotalRewardsResponse = await distribution.query.DelegationTotalRewards({ delegatorAddress: delegation.delegatorAddress });
      // console.log("queryDelegationTotalRewardsResponse");
      // console.log(queryDelegationTotalRewardsResponse);

      let queryDelegatorValidatorsResponse = await distribution.query.DelegatorValidators({ delegatorAddress: delegation.delegatorAddress });
      // console.log("queryDelegatorValidatorsResponse");
      // console.log(queryDelegatorValidatorsResponse);

      let queryDelegatorWithdrawAddressResponse = await distribution.query.DelegatorWithdrawAddress({ delegatorAddress: delegation.delegatorAddress });
