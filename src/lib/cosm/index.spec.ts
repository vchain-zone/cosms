...
describe('Cosm test', async () => {
  ...
  describe('Get denom', async () => {

    it('should get mint denom', async function() {
      let bank = cosm.cosmos.bank;
      let mint = cosm.cosmos.mint;
      let mintDenom = await mint.query.Params({});
      let mintDenonStr = mintDenom.params.mintDenom;

      let denom = await bank.query.DenomMetadata({ denom: mintDenonStr });
