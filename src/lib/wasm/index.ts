...
import { Wasm as CosmosWasm } from '../cosmos/wasm';
...
export class StaticWasm {
  ...
  private wasm: CosmosWasm;

  constructor(provider: Provider) {
    this._provider = provider;
    this.wasm = new CosmosWasm(provider);
  }

  public async query(queryMessage: QueryMessage, height?: number): Promise<JsonObject> {
    await this.checkCosmWasmClient();

    // return await this._cosmWasmClient.queryContractSmart(  queryMessage.contractAddr,
    //   queryMessage.queryMsg)
    return await this.wasm.queryContractSmart(
