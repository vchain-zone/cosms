...
export class Wallet {
  ...
  public static async getWalletFromKey(provider: Provider,
                                       privkey: string,
                                       options?: WalletOptions) {
    if (!privkey) throw Error(`privkey must be an hex string, not be ${privkey} `);
    const key = fromHex(privkey);

    const signer = await DirectSecp256k1Wallet.fromKey(key, provider.bech32Prefix);
    return Wallet.getWalletsFromOfflineSigner(provider, signer, options);
  }
